    ...
    <div class="tagView_utils">
      ...
      <div class="tagView_scroll" >
        <el-scrollbar @wheel.prevent="onmousewheel" ref="scrollContainer">
          <div class="tagView">
            <div class="tagView_item"
              :class="{'is-active': path === el.name}"
              @click.left="open(el.name)"
              @click.prevent.right="contextmenu($event, el.name)"
              v-for="el in tagViews" :key="el.name">{{el.meta.title}}
              <el-icon v-if="!el.meta.isFixed" @click="cliseCurrent(el.name)"><Close /></el-icon>
            </div>
          </div>
        </el-scrollbar>
      </div>
      <!-- <el-button icon="Right" size="small"/> -->

      <Teleport to="body">
        <ul v-if="isShow" class="el-dropdown-menu" :style="{'left': left + 'px', 'top': top + 'px'}">
          <li class="el-dropdown-menu__item" @click.stop="refresh">刷新</li>
          <li class="el-dropdown-menu__item" @click="cliseCurrent">关闭当前</li>
          <li class="el-dropdown-menu__item" @click="rest">关闭其他</li>
        </ul>
      </Teleport>
...
let onmousewheel = ($event, el) => {
  scrollWrap.value.scrollLeft = scrollWrap.value.scrollLeft - $event.wheelDelta / 4
}
...
// 关闭当前页
let cliseCurrent = (name) => {
  base.delTagView((name || activeName))
  isShow.value = false;
}
// 关闭其他页
let rest = () => {
